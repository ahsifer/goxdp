...
2. Start the slave service <br>
   `docker run -d --network host --name goxdp-slave --privileged --restart always -v /etc/localtime:/etc/localtime:ro ahsifer/goxdp-dev:3.0 server --master=true --masterIP=127.0.0.1 --masterPort=9999 --validSSL=false --masterPullInterval=5 --protocol=https --timeoutInterval=5`
